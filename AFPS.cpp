...
AFPS::AFPS(Game* game)
	:AActor(game)
{
	mMoveComp = new CMove(this);
	mAudioComp = new CAudio(this);
	mLastFootstep = 0.0f;
	mFootstep = mAudioComp->PlayEvent("event:/Footstep");
	mFootstep.SetPaused(true);

	mCameraComp = new CFPSCamera(this);

	//mFPSModel = new AActor(game);
	//mFPSModel->SetScale(0.75f);
	//mMeshComp = new CMesh(mFPSModel);
	//mMeshComp->SetMesh(game->GetRenderer()->GetMesh("Assets/Rifle.gpmesh"));

	// Add a box component
	mBoxComp = new CBox(this);
...
void AFPS::UpdateActor(float deltaTime)
{
...
void AFPS::ActorInput(const uint8_t* keys)
{
...
void AFPS::SetFootstepSurface(float value)
{
...
void AFPS::SetVisible(bool visible)
{
	mMeshComp->SetVisible(visible);
}

void AFPS::FixCollisions()
{
