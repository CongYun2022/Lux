...
unsigned int SAudio::sNextID = 0;

SAudio::SAudio(Game* game)
	:mGame(game)
	,mSystem(nullptr)
...
void SAudio::LoadBank(const std::string& name)
{
...
void SAudio::UnloadBank(const std::string& name)
{
...
SoundEvent SAudio::PlayEvent(const std::string& name)
{
...
void SAudio::SetListener(const Matrix4& viewMatrix)
{
...
float SAudio::GetBusVolume(const std::string& name) const
{
	float retVal = 0.0f;
...
bool SAudio::GetBusPaused(const std::string & name) const
{
	bool retVal = false;
	const auto iter = mBuses.find(name);
	if (iter != mBuses.end())
...
void SAudio::SetBusVolume(const std::string& name, float volume)
{
...
void SAudio::SetBusPaused(const std::string & name, bool pause)
{
	auto iter = mBuses.find(name);
	if (iter != mBuses.end())
	{
...
FMOD::Studio::EventInstance* SAudio::GetEventInstance(unsigned int id)
{
	FMOD::Studio::EventInstance* event = nullptr;
